...
class TaskFilterTest(TestCase):
    ...
    def test_filter_active_logic(self):
        qs = TaskFilter(
            data={"active_filter": "active"}, queryset=Task.objects.all()
        ).qs
        self.assertIn(self.task_today, qs)
        ...
        qs = TaskFilter(
            data={"active_filter": "deactive"}, queryset=Task.objects.all()
        ).qs
        self.assertEqual(qs.count(), 1)
    ...
    def test_filter_deadline_dates(self):
        qs = TaskFilter(
            data={"deadline_filter": "today"}, queryset=Task.objects.all()
        ).qs
        self.assertIn(self.task_today, qs)
        self.assertEqual(qs.count(), 1)

        qs = TaskFilter(
            data={"deadline_filter": "overdue"}, queryset=Task.objects.all()
        ).qs
        self.assertIn(self.task_overdue, qs)

        qs = TaskFilter(
            data={"deadline_filter": "tomorrow"}, queryset=Task.objects.all()
        ).qs
        self.assertIn(self.task_tomorrow, qs)
    ...
    def test_filter_this_week_logic(self):
        ...
        if days_until_sunday >= 2:
            ...
            qs = TaskFilter(
                data={"deadline_filter": "this_week"}, queryset=Task.objects.all()
            ).qs
            self.assertIn(sunday_task, qs)

    def test_filter_by_direct_fields(self):
        qs = TaskFilter(
            data={"priority": Priority.HIGH}, queryset=Task.objects.all()
        ).qs
        self.assertIn(self.task_today, qs)
        self.assertEqual(qs.count(), 1)

        qs = TaskFilter(
            data={"task_type": self.type_bug.id}, queryset=Task.objects.all()
        ).qs
        self.assertIn(self.task_overdue, qs)
